...
class MyFlatPushButton(QtGui.QPushButton):
    def __init__(self, caption, min_size=(75, 75)):
...
class SoftInputWidget(QtGui.QDialog):
    ...
    def do_layout(self, keyboard_type='default'):
       ...
       """
        ...
        alist = list()
        alist.append((QtCore.Qt.Key_Left, '<-'))
        alist.append((QtCore.Qt.Key_Right, '->'))
        alist.append((QtCore.Qt.Key_Up, '^'))
        alist.append((QtCore.Qt.Key_Down, 'v'))
        for key in alist:
...
class MyWindowClass(TouchInputWidget):
    ...
    def cf_clicked(self):
        cel = float(self.inputDialog.text())
        fahr = cel * 9 / 5.0 + 32
        self.outputDialog.setText(str(fahr + 0.5))
        SoftInputWidget._input_panel_all.show_input_panel(widget)

    def fc_clicked(self):
        fahr = float(self.inputDialog.text())
        cel = (fahr - 32) * 5 / 9
        self.outputDialog.setText(str(cel))
